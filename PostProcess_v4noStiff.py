...
#This script is an update version of the script PostProcess_v4noStiff2.py
...
for p in range(15):
	...
	f=open('vonMiseMax_'+str(p)+'.txt','w')
	Samp=open('Ampl_Stress'+str(p)+'.txt','w')
	Se=open('Mean_Princ_Stress'+str(p)+'.txt','w')
	Seff=open('Eff_Stress'+str(p)+'.txt','w')
	Meff=open('Mean_Stress'+str(p)+'.txt','w')
	Nc=open('N_cycles'+str(p)+'.txt','w')
	vm=open('vonMises'+str(p)+'.txt','w')
	
	#call the files that has the elements of maximum principal stresses as well as von Mises stresses
	fh = open('C:\Users\celestink\Documents\TestingStation\EllptContact3\StiffInclusions\RailMaxPrinc_'+str(p)+'.txt')
	x = []
	xv=[]
	...
	#Create a file that contain maximum stresses for every every standard deviation given to random stiffness in the ties
		...
		#Maximum Mises:
		InclElemvon=odb1.rootAssembly.elementSets['NewElSetsvoM2']
		stresses=odb1.steps['Step-1'].frames[-1].fieldOutputs['S']
